            ...
            {[...receiptList].reverse().map((r, i) => (
              <div key={i} className={`flex items-center justify-between px-2.5 py-2 rounded-lg text-xs ${r.succeeded ? 'bg-[#2EE59D]/5' : 'bg-red-500/5'}`}>
                <div className="flex items-center gap-1.5">
                  ...
                  {tokenIds && (tokenIds as bigint[])[i] && <a href={`https://opensea.io/assets/base/${contracts.vaadaReceipts}/${Number((tokenIds as bigint[])[i])}`} target="_blank" rel="noopener noreferrer" className="text-[9px] font-medium px-1.5 py-0.5 rounded-full border border-[#2EE59D]/30 text-[#2EE59D] hover:bg-[#2EE59D]/10 transition-colors ml-1.5 whitespace-nowrap">View Receipt</a>}
                </div>
